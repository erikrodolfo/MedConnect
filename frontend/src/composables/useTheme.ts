...
export function useTheme() {
  ...
  const applyTheme = (theme: Theme): void => {
    //função para acessar o html da página e adicionar um atributo
    document.documentElement.setAttribute("data-theme", theme); //esse atributo
  };

  onMounted(() => {
    //ativa assim que o componente aparece na tela
    const saved = localStorage.getItem("theme") as Theme | null; //pega o tema salvo no navegador

    if (saved === "light" || saved === "dark") {
      //verificando se o valor é válido/ se não for, usa o tema do sistema
      store.setTheme(saved);
    } else {
      const prefersDark = window.matchMedia(
        "(prefers-color-scheme: dark)",
      ).matches; //verificando preferência do sistema operacional
  ...
  return {
    theme: computed(() => store.theme), //expõe o valor atual
